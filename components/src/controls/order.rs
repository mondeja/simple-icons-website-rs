...
pub fn set_order_mode_on_localstorage(order_mode: OrderMode) {
    let window = web_sys::window().unwrap();
    let local_storage = window.local_storage().unwrap().unwrap();
    local_storage
        .set_item(
        ...
        <div class="flex flex-col">
            ...
            <div class="flex flex-row">
                <ControlButtonSVGPath
                    ...
                    active=move || {controls_state().order_mode == OrderMode::Alphabetic}
                    on:click=move |_| {
                        use log::info;
                        info!("Order by alphabetic");
                        controls_state.update(|state| {
                            state.set_order_mode(OrderMode::Alphabetic);
                        });
                    }
                />
                <ControlButtonSVGPath
                    ...
                    active=move || {controls_state().order_mode == OrderMode::Color}
                    on:click=move |_| {
                        controls_state.update(|state| {
                            state.set_order_mode(OrderMode::Color);
                        });
                    }
                />
            </div>
