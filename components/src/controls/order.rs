...
pub fn set_order_mode(
    ...
    order_mode_signal.update(move |mut state| {
    ...
    if update_grid {
        icons_grid_signal.update(move |grid| grid.set_order_mode(order_mode));
    }
}
                            ...
                            .push(
                                view! { cx,
                                    <ControlButtonSVGPath
                                        title=move_gettext!(cx, "Sort by search match score")
