...
impl SVGDefs {
    pub fn id(&self) -> &'static str {
        match self {
            Self::ViewPath => Ids::ViewSVGPath.as_str(),
            Self::DownloadPath => Ids::DownloadSVGPath.as_str(),
            Self::ControlsPath => Ids::ControlsSVGPath.as_str(),
            Self::CrossPath => Ids::CrossSVGPath.as_str(),
        }
    }
    ...
    view! { cx,
        <svg class="hidden">
            <defs>
                ...
                <path id=SVGDefs::DownloadPath.id() d=SVGDefs::DownloadPath.d()></path>
                <path id=SVGDefs::ControlsPath.id() d=SVGDefs::ControlsPath.d()></path>
                <path id=SVGDefs::CrossPath.id() d=SVGDefs::CrossPath.d()></path>
            </defs>
        </svg>
    }
}
